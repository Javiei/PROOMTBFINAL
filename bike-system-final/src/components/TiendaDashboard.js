...
export const TiendaDashboard = () => {
  ...
  // Bicicletas que la tienda debe recibir (status 'tienda')
  const bikesToReceive = bikes.filter(bike => bike.status === 'tienda');
  // Bicicletas que la tienda debe terminar (status 'listo_tienda')
  const bikesToFinish = bikes.filter(bike => bike.status === 'listo_tienda');
  ...
  const handleNewEntry = async () => {
    ...
    const newBike = {
      ...
      status: 'tienda', // Se asigna a la tienda para que la reciba
      assignedTo: 'tienda1',
    ...
    setShowForm(false);
    alert(`¡Bici de ${clientName} ${clientLastName} registrada con éxito y asignada para recibir!`);
  };
  ...
  const handleFinishProcess = async (bike) => {
    ...
    alert(`Proceso de la bicicleta de ${bike.clientName} terminado. ¡Cliente satisfecho!`);
  };

  const filteredBikesToReceive = bikesToReceive.filter(bike =>
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <h2 className="text-3xl font-bold text-gray-900 mb-8">Panel de la Tienda</h2>
      ...
      <h3 className="text-2xl font-bold text-gray-900 mb-6">Bicicletas por Recibir (Entrada)</h3>
      {filteredBikesToReceive.length === 0 ? (
        <p className="text-gray-600 text-lg">No hay bicicletas pendientes para recibir. ¡Todo en orden!</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredBikesToReceive.map(bike => (
